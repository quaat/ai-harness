...
async function commitTask(taskId: string, opts: { phase: string; checks?: boolean }) {
  ...
  if (opts.checks !== false) {
    ...
    if (results.some((r) => !r.ok)) throw new Error(`Checks failed. See ${t.artifacts.tests}`);
...
async function prTask(taskId: string, opts: { draft?: boolean; skipReview?: boolean }) {
  ...
  if (!afterArtifactsWriteClean) {
    throw new Error(`Generated PR artifacts changed the working tree. Commit ${t.artifacts.pr} and ${path.join(path.dirname(t.artifacts.pr), "task.yaml")} first, then rerun task pr.`);
  }
